...
a {text-decoration:underline;color:white;}
  ...
  File.foreach(with_path).with_index do |line, line_num|
    ...
    if comment_line
      ...
      html_out << "<code title='Line #{line_num + 1}, #{with_path}'><pre><a name='#{line_num + 1}'  class='githubline' href='https://github.com/ukparliament/egg-timer/tree/master/#{with_path}#L#{line_num + 1}'> #{line_num + 1}</a> " << line << '</pre></code>'
      markdown_out << line
    end
  end
